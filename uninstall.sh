...
Uninstall_FX()
{
  ...
  trace_log_message "${bg_green}Uninstalling the FX Agent ...${clear}"
...
Uninstall_VX()
{  
  trace_log_message "${bg_green}Uninstalling the VX agent ...${clear}"
...
if [ -z "$FX_AGENT_FOUND" -a -z "$VX_AGENT_FOUND" ]; then
    trace_log_message "${bg_red}Either FX or VX is not installed in the setup ...${clear}"
